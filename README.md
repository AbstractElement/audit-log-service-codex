...
When a turn touches any file under `.specs/<feature>/`, the hook requires a same-day `.specs/<feature>/eval-report-<date>.md` generated by the `spec-self-eval` skill. It blocks turn closure when the report is missing, stale, has a `FAIL` verdict, contains a `[FAIL]` row, or has any category score from `1` through `3`.
