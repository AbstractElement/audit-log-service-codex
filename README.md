...
### Codex and Claude Stop hooks for specs

Repo-local Codex hook configuration lives in `.codex/config.toml` with `codex_hooks` enabled. The configured `Stop` hook runs `.codex/hooks/spec_self_eval_stop.py` before a Codex turn closes. The same guard is mirrored for Claude through `.claude/settings.json`, which runs `.claude/hooks/spec_self_eval_stop.py`.

When a turn edits or inspects any concrete file under `.specs/<feature>/`, or when Git reports uncommitted `.specs` changes, the hook requires the latest `.specs/<feature>/eval-report-YYYY-MM-DD.md` or `.specs/<feature>/eval-report-YYYY-MM-DD-HHMMSS.md` generated by the `spec-self-eval` skill to be newer than `requirements.md`, `design.md`, and `tasks.md`. It blocks turn closure when the report is missing, older than the spec files, has a `WEAK` or `FAIL` verdict/status, or has any category score from `0` through `3`.

Manual checks:

```bash
SPEC_SELF_EVAL_FEATURES=query-api .codex/hooks/spec_self_eval_stop.py
SPEC_SELF_EVAL_FEATURES=query-api .claude/hooks/spec_self_eval_stop.py
```

Use `SPEC_SELF_EVAL_FEATURES=<feature>` to force a feature folder during local verification without editing files.
