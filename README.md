...
### Codex Stop hook for specs

Repo-local Codex hook configuration lives in `.codex/config.toml` with `codex_hooks` enabled. The configured `Stop` hook runs `.codex/hooks/spec_self_eval_stop.py` before a Codex turn closes.
