...
When a turn edits or inspects any concrete file under `.specs/<feature>/`, or when Git reports uncommitted `.specs` changes, the hook requires a same-day `.specs/<feature>/eval-report-YYYY-MM-DD.md` or `.specs/<feature>/eval-report-YYYY-MM-DD-HHMMSS.md` generated by the `spec-self-eval` skill. It blocks turn closure when the report is missing, older than `requirements.md`, `design.md`, or `tasks.md`, has a `WEAK` or `FAIL` verdict/status, or has any category score from `0` through `3`.
